...
class EntityBreedingExecutor(
    ...
    override fun execute(entity: EntityCreature): Boolean {
        ...
        if (spouse == null || spouse!!.isDead) {
            spouse = (entity.behaviorGroup.memoryStorage.get(MemoryTypes.ENTITY_SPOUSE) as? AnimalEntity)
                ?.takeIf { !it.isDead }
                ?: findSpouse(entity).also { found ->
                    if (found != null) {
                        isInitiator = true
                        entity.behaviorGroup.memoryStorage.set(MemoryTypes.ENTITY_SPOUSE, found)
                        (found as? IntelligentEntity)?.behaviorGroup?.memoryStorage?.set(MemoryTypes.ENTITY_SPOUSE, entity)
                    }
                }
        }
    ...
    override fun onStop(entity: EntityCreature) {
        ...
        entity.behaviorGroup.memoryStorage.clear(MemoryTypes.ENTITY_SPOUSE)
        spouse = null
        isInitiator = false
    }
        ...
        return entity.instance?.entities
            ...
            ?.filter { candidate ->
                candidate !== entity &&
                        ...
                        candidate.behaviorGroup.memoryStorage.get(MemoryTypes.ENTITY_SPOUSE) == null &&
