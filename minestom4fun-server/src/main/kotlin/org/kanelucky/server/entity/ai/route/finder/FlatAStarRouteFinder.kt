package org.kanelucky.server.entity.ai.route.finder

import net.minestom.server.entity.EntityCreature
import net.minestom.server.instance.Instance
import org.kanelucky.server.entity.ai.route.Node
import org.kanelucky.server.entity.ai.route.posevaluator.GroundPosEvaluator
...
open class FlatAStarRouteFinder(
    ...
    protected open fun doSearch(entity: EntityCreature, tx: Double, ty: Double, tz: Double): List<Node> {
        ...
        val startX = floor(pos.x()).toInt()
        val startZ = floor(pos.z()).toInt()
        val endX = floor(tx).toInt()
        val endZ = floor(tz).toInt()

        val startNode = Node(startX + 0.5, pos.y(), startZ + 0.5)
        val endNode = Node(endX + 0.5, ty, endZ + 0.5)
        ...
        if (bestNode !== startNode) return reconstructPath(bestNode)
        return emptyList()
    }
    ...
    protected open fun getNeighbors(current: Node, instance: Instance, entity: EntityCreature): List<Node> {
        ...
        for (offset in FLAT_NEIGHBORS) {
            ...
            for (dy in 1 downTo -maxFallDistance) {
                val ny = cy + dy
                if (isWalkable(nx, ny, nz, instance, entity)) {
                    neighbors.add(Node(nx + 0.5, ny.toDouble(), nz + 0.5))
                    break
                }
            }
        }
